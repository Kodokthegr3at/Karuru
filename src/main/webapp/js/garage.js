...
async function loadGarageProducts() {
    const container = document.getElementById('garageProducts');
    const emptyState = document.getElementById('emptyGarage');
    ...
    try {
        ...
        const url = `${contextPath}/GarageServlet`;
        console.log('Fetching garage products from:', url);
        ...
        console.log('Garage data received:', data);
        ...
        if (data && data.success !== false && data.products && Array.isArray(data.products) && data.products.length > 0) {
            ...
            renderGarageProducts(data.products);
        ...
        console.error('Error loading garage products:', error);
            ...
            container.innerHTML = `
                <div class="col-12">
                    <div class="text-center py-5 text-danger">
                        ...
                        <button onclick="loadGarageProducts()" class="btn btn-primary mt-3">
...
function renderGarageProducts(products) {
    const container = document.getElementById('garageProducts');
    if (!container) return;
    ...
    container.innerHTML = products.map(product => {
        ...
        const imageUrl = KaruruUtils.resolveProductImageUrl(firstImage);
                ...
                <a href="${productUrl}" class="product-card-link text-decoration-none">
                    <div class="card card-light h-100 product-card-modern">
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <div class="mt-2">
                                <span class="badge ${getStatusBadgeClass(product.order_item_status || product.order_status)} mb-2">
                                    ${getStatusLabel(product.order_item_status || product.order_status)}
                                </span>
                                <small class="text-muted d-block">
                                    <i class="bi bi-calendar-check me-1"></i>
                                    購入日: ${formatDate(product.purchase_date || product.order_created_at || product.created_at)}
                                </small>
                                ${product.order_number ? `
                                    <small class="text-muted d-block mt-1">
                                        <i class="bi bi-receipt me-1"></i>
                                        注文番号: ${escapeHtml(product.order_number)}
                                    </small>
                                ` : ''}
                                <button type="button" class="btn btn-sm btn-outline-primary mt-2 w-100" onclick="event.stopPropagation(); event.preventDefault(); window.location.href='${contextPath}/order-detail.jsp?id=${product.order_id}'">
                                    <i class="bi bi-eye me-1"></i>注文詳細
                                </button>
                            </div>
                        </div>
                    </div>
